...
// Realtime validation helper (PRD 6.2.1)
watch(() => form.username, (val) => {
  if (val && !/^[a-zA-Z0-9_]{4,20}$/.test(val)) {
    validationErrors.username = '用户名必须是 4~20 位字母、数字或下划线'
  } else {
...
watch(() => form.email, (val) => {
  if (val && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) {
    validationErrors.email = '请输入合法的邮箱格式'
  } else {
...
watch(() => form.password, (val) => {
  if (val && (val.length < 8 || val.length > 32 || !/[a-zA-Z]/.test(val) || !/[0-9]/.test(val))) {
    validationErrors.password = '密码需 8~32 位且至少包含字母与数字'
  } else {
    validationErrors.password = ''
  }
  if (form.confirmPassword && val !== form.confirmPassword) {
    validationErrors.confirmPassword = '确认密码与新密码不一致'
  } else {
...
const handleSubmit = async () => {
  ...
  if (validationErrors.username || validationErrors.email || validationErrors.password || validationErrors.confirmPassword) {
    errorMsg.value = '表单内容填写有误，请修正后再提交'
    return
  }
  ...
  try {
    await authStore.register(form.username, form.email, form.nickname, form.password)
    
    // Dispatch success toast
    const event = new CustomEvent('app-toast', { detail: { text: '注册成功！请使用刚才注册的账号和密码登录。' } })
    window.dispatchEvent(event)

    router.push('/login')
  } catch (err: any) {
    errorMsg.value = err.message || '注册失败，该用户名或邮箱已被占用'
  } finally {
    loading.value = false
  }
}
